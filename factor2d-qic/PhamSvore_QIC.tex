...
\end{displaymath}
\centerline{}
\fcaption{Circuit resources for the constant-depth teleportation circuit
shown in Figure \ref{fig:cdt} for transporting a single qubit across
$n$ qubits.}
\label{tab:cdt}
\end{center}
\end{table}
...
\end{displaymath}
\centerline{}
\fcaption{Circuit resources for the constant-depth fanout circuit
shown in Figure \ref{fig:cdf} for creating $n$ entangled copies of a
single qubit.}
\label{tab:cdf}
\end{table}
...
\end{displaymath}
\centerline{}
\fcaption{Circuit resources for un-fanout circuit consisting of
...
\end{displaymath}
\centerline{}
\fcaption{Circuit resources for the carry-save adder circuit from Figure \ref{fig:csa-circuit}.}
...
\end{displaymath}
\centerline{}
\fcaption{Circuit resources for the Toffoli circuit given in Figure \ref{fig:toffoli}.}
\label{tab:toffoli}
\end{table}
...
\end{displaymath}
\centerline{}
\fcaption{Circuit resources for the $n$-bit modular $3$-to-$2$ adder represented
by Figure \ref{fig:csa-add-4}.}
...
architecture for $t'=18$ in a logarithmic-depth tree with height $(\lceil \log_{\frac{3}{2}}(t'/3) \rceil + 1) = 6$.}
\label{fig:mod-mult}
\end{figure*}
%
...
\hline
Fowler, Devitt, Hollenberg \cite{Fowler2004} & \textsc{1D NTC}   & $O(n^3)$ & & $O(n)$\\
...
Current Work               & \textsc{2D CCNTCM}   & $O(\log^2{n})$ & & $O(n^4)$   \\
\hline
\end{tabular}
\end{center}
