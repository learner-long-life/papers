...
architectures, achieving a depth of $O(\log^3 n)$ and a width of $O(n^4)$ qubits, as detailed in Table
...
\textsc{CCAC} and \textsc{CCNTC}, respectively, with definitions
...
\begin{itemize}
\item a planar graph $(V,E)$ where $V$ represents all qubits and $E$ represents
allowed two-qubit interactions between qubits
\item a universal gate set ${X, Z, H, T, T^{\dagger}, CNOT}$
\item a deterministic machine (classical controller) $M$ that applies 
\end{itemize}
...
The third extension, and the most significant, is to consider
...
entanglement (citation needed, scalable NIST architecture, or MUSIQC architecture). We call these individual machines \emph{modules}, each of
...
controlled NOT (CNOT) gate), the $T$ gate and its inverse $T^{\dagger}$, 
and single-qubit measurements (in the $Z$-basis).
Together, all of these except the last form a universal set of unitary
gates \cite{Kitaev2002}.

A measurement in the Bell basis we interpret as having depth $4$,
size $4$, and width $2$ according
...
which cannot be performed transversally in many popular error-correcting codes
\cite{Fowler2011}.
...
\end{displaymath}
\centerline{}
\fcaption{Circuit resources for teleportation, fanout, and un-fanout
(consisting of
...
\begin{figure*}[h!bt]
\centerline{
\includegraphics[width=3in]{./csa-tile-symbol.pdf}
}
...
The detailed resources are listed in Table \ref{tab:csa-tile-resources},
which is twice the resources described in the previous two paragraphs due
to reversing the computation. There is one exception to this.
...
layers of constant-depth teleportation and CNOT.
...
\end{displaymath}
\centerline{}
\fcaption{Circuit resources for modular addition.}
...
quantum integers, which requires $(t'-2)$ modular additions, for $t'=n^2$.
...
\begin{equation}
D_{MM} = 3.4 \log_3^2 n + 1295 \log_2 n + 6911
\end{equation}
...
is $O(\log^2 n)$ and the width is $O(n^4)$. We will now calculate numerical
...
\begin{equation}
4 \log_2^2 n - (16n + 30)\log_2 n + 16n^2 + 60n + 56
\end{equation}
...
\begin{equation}
D_{ME} = 3.4\log_2^3 n + 1337\log_2^2 n + 23070\log_2 n + 86279
\end{equation}
...
S_{ME} & = & 96 \log_2^3 n + \nonumber \\
       ...
       &   & 3520185 n^4 + 29097629 n^3 + 41260290 n^2 + 13300960 n - 4645
\end{eqnarray}
...
probability for each round of period-finding. Not all resources are
provided directly by the referenced source.

Resources in square brackets
are inferred using the observation that circuit size is bounded above by
the product of circuit depth and circuit width. Moreover, the maximum
depth of a circuit is the size, since all gates can be applied in sequence
in the worst case. These upper bounds are correct,
...
\end{tabular}
\end{center}
\fcaption{Asymptotic circuit resource usage for quantum factoring of an $n$-bit number.}
\label{tab:results}
\end{table*}
