...
& \lstick{\ket{\ell}}	& \qw & \gate{H} & \qw & \qw      & \qw & \targfix & \qw & \gate{H} & \qw & \lstick{Z^{j_2 \oplus j_4}(\alpha\ket{0} + \beta\ket{1})}
}
\end{displaymath}
\centerline{}
\fcaption{A novel, constant-depth circuit for unbounded quantum unfanout on
2D CCNTC, from the $7$-qubit entangled state $\alpha\ket{0}^{\otimes 7} + \beta\ket{1}^{\otimes 7}$ to the
...
consisting of all $\ket{0}$'s except for a single qubit $\alpha\ket{0} + \beta\ket{1}$, which is in the
...
operations. In the 2D CCNTC model, the operations are not identical due to
the one-way nature of the measurement and Pauli corrections and the
constraints of NTC. In Figure \ref{fig:cdu}, we present a novel quantum circuit for unbounded quantum
unfanout for $n=7$ in constant depth on 2D CCNTC.
Note that the state in \ref{eqn:fanned-out}
...
which in this case is the bottom qubit in the figure.
...
next-to-last qubit $j_6$. Each measurement projects the state of the
target qubit
...
initial qubit $\alpha\ket{0} + \beta\ket{1}$.
...
neighbors. The unfanout circuit in Figure \ref{fig:cdu} is the inverse of the
fanout circuit in 
